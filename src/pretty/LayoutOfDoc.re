...
let rec layout_of_doc': 'annot. Doc.t('annot) => Doc.m(Layout.t('annot)) =
  doc => {
    let g = (~width: int, ~pos: int): Doc.m'(Layout.t('a)) => {
    ...
    let h = (~width: int, ~pos: int): Doc.m'(Layout.t('annot)) => {
      let key = (width, pos);
